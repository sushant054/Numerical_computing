...
// Default constructor
Matrix::Matrix() {
    mat = nullptr; // Initialize the matrix pointer to nullptr
...
Matrix::Matrix(int r, int c) : rows(r), cols(c) {
    ...
    for (int t = 0; t < rows; t++) {
        mat[t] = new double[cols]; // Allocate memory for columns in each row
    }
...
void Matrix::display() {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            std::cout << mat[i][j] << "\t"; // Print each element with a tab
        }
    ...
    if (mat != nullptr) {
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < cols - 1; c++) {
                if (r != c) {
                    sum += fabs(mat[r][c]); // Sum the absolute values of non-diagonal elements in the row
                }
            }
            if (fabs(mat[r][r]) <= sum) // Check if diagonal element is greater than or equal to the sum
                return false;
        }
        return true;
    }
...
bool Matrix::makeDiagonallyDominant() {
    for (int r = 0; r < rows; r++) {
        ...
        for (int c = 0; c < cols - 1; c++) {
            if (r != c)
                s += fabs(mat[r][c]); // Sum the absolute values of non-diagonal elements in the row
        }
        if (s > fabs(mat[r][r])) { // If the row is not diagonally dominant
...
int Matrix::getDDRow(int row) {
    for (int i = row + 1; i < rows; i++) {
        ...
        for (int j = 0; j < cols - 1; j++) {
            if (row != j)
                s += fabs(mat[i][j]); // Sum the absolute values of non-diagonal elements in the row
        }
...
void Matrix::guassSeidel() {
    ...
    // Iteratively apply the Gauss-Seidel method
    while (flag) {
        for (int i = 0; i < cols - 1; i++) {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < cols - 1; c++) {
                if (r != c) {
                    lhs += mat[r][c] * var[c]; // Calculate the sum of the known terms
                }
            }
    ...
    std::cout << "Iterations for Seidel ::" << count << std::endl;
    for (int i = 0; i < cols - 1; i++) {
